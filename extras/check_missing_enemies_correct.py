#!/usr/bin/env python3
"""
Check missing enemies - use persona_name field for image matching
"""
import os
import json
    ...
    for game_name, (json_path, folder_id) in games.items():
        ...
        with open(json_path, 'r', encoding='utf-8') as f:
            ...
            if isinstance(data, list):
                ...
                enemies = list(data.values()) if isinstance(next(iter(data.values())), dict) else data
        ...
        for enemy in enemies:
            if isinstance(enemy, dict):
                # Use persona_name if available, otherwise use name
                persona_name = enemy.get('persona_name', enemy.get('name', ''))
                if persona_name and persona_name not in unique_personas:
        ...
        coverage = ((len(unique_personas) - len(missing)) / len(unique_personas) * 100) if unique_personas else 0
        print(f"{game_name}: {len(unique_personas) - len(missing)}/{len(unique_personas)} ({coverage:.1f}%)")
...
def main():
    print("=== ACTUAL MISSING ENEMIES (by persona name) ===\n")
    ...
    if total_missing == 0:
        ...
        if total_missing <= 20:
            print("\n=== ALL MISSING ENEMIES ===")
            for game, name in sorted(all_missing):
                print(f"  {game}: {name}")
